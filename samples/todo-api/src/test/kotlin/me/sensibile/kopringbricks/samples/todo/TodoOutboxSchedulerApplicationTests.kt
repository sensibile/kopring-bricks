...
class TodoOutboxSchedulerApplicationTests {
    ...
    private fun awaitPublishedEvents(count: Int): List<OutboxEvent> {
        ...
        var publishedEvents = outboxPublisher.events
        var storedEvents = outboxRepository.events

        while (System.nanoTime() < deadline) {
            val storedPublishedCount = storedEvents.count { it.status == OutboxEventStatus.PUBLISHED }

            if (publishedEvents.size >= count && storedPublishedCount >= count) {
                return publishedEvents
            }

            Thread.sleep(AWAIT_INTERVAL.toMillis())
            publishedEvents = outboxPublisher.events
            storedEvents = outboxRepository.events
        }
