...
class Resilience4jEnvironmentPostProcessorTests {
    ...
    fun `adds resilience4j defaults`() {
        ...
        assertEquals(100, environment.getProperty("resilience4j.circuitbreaker.configs.default.sliding-window-size", Int::class.java))
        assertEquals(3, environment.getProperty("resilience4j.retry.configs.default.max-attempts", Int::class.java))
        assertEquals(Duration.ofSeconds(2), environment.getProperty("resilience4j.timelimiter.configs.default.timeout-duration", Duration::class.java))
        assertEquals(true, environment.getProperty("management.health.circuitbreakers.enabled", Boolean::class.java))
        assertEquals(true, environment.getProperty("resilience4j.circuitbreaker.configs.default.register-health-indicator", Boolean::class.java))
    }
    ...
    fun `uses kopring property overrides for generated defaults`() {
        ...
        assertEquals(50, environment.getProperty("resilience4j.bulkhead.configs.default.max-concurrent-calls", Int::class.java))
    }
